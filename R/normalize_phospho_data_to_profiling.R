...
normalize_phospho_data_to_profiling <- function (phospho_data_normalized, profiling_data_normalized,
  ...
  if(!pair_flag){
    if(is.na(control_label)){ # no control, nomalization with median
      ...
      phospho_data_normalize_by_column <- normalize_to_column_median_noPair_noControl(phospho_data_normalized)
      # normalize profiling
      profiling_data_normalizedby_column <- normalize_to_column_median_noPair_noControl(profiling_data_normalized)
    }else{ # having control, nomalization with median
      cat('\n', 'No pair and having control, nomalization with median of control')
      phospho_data_normalize_by_column <- normalize_to_column_control_noPair_Control(phospho_data_normalized, phosphorylation_experiment_design_file, control_label)
      profiling_data_normalizedby_column <- normalize_to_column_control_noPair_Control(profiling_data_normalized, profiling_experiment_design_file, control_label)
    }
  }else{
