...
# load PhosMap
library('PhosMap')
# load intermediate results from demo
data(BRAFi)
# A data frame for motif enrichment analysis as background
background_df <- BRAFi$background_df
# A data frame containning phosphoproteomics data normalized by proteomics data.
data_frame_normalization_with_control_no_pair <- BRAFi$data_frame_normalization_with_control_no_pair
# A data frame for motif enrichment analysis as foreground.
foreground_df <- BRAFi$foreground_df
# A data frame for time course analysis as input.
fuzzy_input_df <- BRAFi$fuzzy_input_df
# A merged phosphoproteomics data frame based on peptides files (unique ID).
merge_df_with_phospho_peptides <- BRAFi$merge_df_with_phospho_peptides
# A matrix for motif profile.
motif_group_m_ratio_df_mat <- BRAFi$motif_group_m_ratio_df_mat
# A phosphoproteomics data frame after normalization and filtering.
phospho_data_normalization_and_filtering_STY <- BRAFi$phospho_data_normalization_and_filtering_STY
# A proteomics data frame after normalization and filtering.
profiling_data_normalized <- BRAFi$profiling_data_normalized
# A data frame that phosphorylation sites had been mapping to protein sequence and eliminated redundancy.
summary_df_of_unique_proteins_with_sites <- BRAFi$summary_df_of_unique_proteins_with_sites
# A factor for experiment group information.
group <- BRAFi$group
...
system.time({
  combined_df_with_mapped_gene_symbol <- get_combined_data_frame_with_mapped_gene_symbol(merge_df_with_phospho_peptides, species = 'human')
})
...
system.time({
  summary_df_of_unique_proteins_with_sites <- get_summary_df_of_unique_proteins_with_sites(combined_df_with_mapped_gene_symbol)
})
...
#  phospho_data_normalization_and_filtering_STY <- get_all_psites_and_normalize_data_with_FOT5(summary_df_of_unique_proteins_with_sites,
#                                                                                             phosphorylation_exp_design_info_file_path)
#  head(phospho_data_normalization_and_filtering_STY)

## ----eval = FALSE--------------------------------------------------------
#  # Based on phospho_data_normalization_and_filtering_STY
#  ID <- paste(phospho_data_normalization_and_filtering_STY$GeneSymbol,
...
#  phospho_data_rownames <- paste(phospho_data_normalization_and_filtering_STY$GI,
...
#  data_frame_normalization_with_control_no_pair <- normalize_phospho_data_to_profiling(
...
## ----eval = FALSE--------------------------------------------------------
#  # (2) SAM
#  sam_results_list <- analysis_deps_sam(expr_data_frame, deps_group, log2_label = FALSE, nperms = 100, rand = NULL, minFDR = 0.05, samr_plot = T)
...
#                                  show_text = F, min_up_text = 15, min_down_text = 15)
...
set.seed(1000)
fuzzy_clustObj <- visualization_line_chart_with_fuzzycluster(fuzzy_input_df, group, group_levels, 
...
# write.csv(clusters_df, 'clusters_df.csv', row.names = T)
...
## ----get_summary_df_list_from_ksea, fig.cap = "KSEA method", fig.width = 6, fig.height = 5, fig.align = 'center'----
system.time({
...
#  kinase_activity_df <- get_kinase_activity_using_mean_or_mlr(ptypes_data, species = 'human', log2_label = TRUE, method = 'mlr')

## ----eval = FALSE--------------------------------------------------------
#  # get kinase activity matrix with mean value method
#  kinase_activity_df <- get_kinase_activity_using_mean_or_mlr(ptypes_data, species = 'human', log2_label = TRUE, method = 'mean')

## ----motif_get_aligned_sequence_df_based_on_fasta_library----------------
# *** foreground ***
foreground_data <- phospho_data_normalization_and_filtering_STY # pre-processed data
...
foreground_df <- motif_get_aligned_sequence_df_based_on_fasta_library(GI, Sequence, AA_in_protein, fixed_length, species)
...
motifs_list <- motif_enrichment_analysis_based_on_background(foreground, AA_in_protein, background, motifx_pvalue)

# Find sequences in foreground that are mapped to specific motif
foreground_sequences_mapped_to_motifs <- get_foreground_sequences_mapped_to_motifs(motifs_list, foreground)


# Find data frame in foreground that are mapped to specific motif
foreground_df_mapped_to_motifs <- get_foreground_df_mapped_to_motifs(foreground_sequences_mapped_to_motifs, foreground, foreground_df)
...
## ----pheatmap, fig.cap = "Assign quantitative values of peptides to their motif",  fig.width = 6, fig.height = 5, fig.align = 'center'----
...
ph <- pheatmap::pheatmap(
  ...
  fontsize_row = 4, cutree_rows = 1, show_rownames = T, cluster_rows = T,
  fontsize_col = 4, cutree_cols = 1, show_colnames = T, cluster_cols = F,
  border_color = 'black', 
  # color = colors, 
  cellwidth = 5, cellheight = 5,
  breaks = breaks,
  ...
  main = 'Kinase-substrate enrichment analysis'
)

## ----formatted_output_foreground_sequences_mapped_to_motifs--------------
# formatting output
formatted_output_df <- formatted_output_foreground_sequences_mapped_to_motifs(foreground_sequences_mapped_to_motifs)
# write file
# write.table(formatted_output_df, 'formatted_output_df.txt', row.names = F, col.names = F, sep = '\t')
